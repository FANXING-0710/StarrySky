...
func Update(delta: float) -> void:
	...
	# 进入 fall 和 run
	if _owner.move_input != 0 and Input.is_action_pressed("right") and Input.is_action_just_released("grab"):
		change_state("Run")
	elif _owner.move_input != 0 and Input.is_action_pressed("left") and Input.is_action_just_released("grab"):
		change_state("Run")
